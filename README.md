...
| B2 | Test OData via Postman | ✅ Completed | All 4 query options demonstrated: `$top`, `$filter`, `$select`, `$orderby`. Screenshots and collection included in `/screenshots`. |
...
| B6 | SAP Build Work Zone | ❌ Not Completed | SAP BTP Trial accounts do not include an SAP Cloud Identity Services tenant, which is a prerequisite for Work Zone subscription. The subscription failed with: "You haven't configured your subaccount to trust a tenant of SAP Cloud Identity Services." This is a known trial account platform limitation and not a candidate error. |

---

## Deployed Application URL
https://a833876atrial.authentication.ap21.hana.ondemand.com/oauth/authorize?response_type=code&client_id=sb-portal-cf-dt-approuter!t101&redirect_uri=https%3A%2F%2Fa833876atrial.cpp.cfapps.ap21.hana.ondemand.com%2Flogin%2Fcallback%3FauthType%3Dxsuaa&state=f22e700c-43e3-41e9-b529-f0613b2b4eac

> **Note:** The application is protected by XSUAA. Accessing the URL redirects to the SAP
> authentication endpoint — this is the expected and intended behaviour as configured during deployment.
